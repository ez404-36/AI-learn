...
|--------|----------------------|
| **LLM** | Нейросеть, предсказывающая следующий токен текста |
| **Токен** | Кусочек текста — единица, которой оперирует модель |
| **Токенизатор** | Компонент, разбивающий текст на токены |
| **Embedding** | Представление текста в виде вектора чисел; близкий смысл → близкие векторы |
| **Transformer** | Архитектура нейросети, на которой построены LLM |
| **Attention** | Механизм, позволяющий учитывать связи между всеми токенами |
| **Авторегрессия** | Генерация ответа по одному токену, подавая вывод обратно на вход |
| **Temperature** | Параметр случайности при выборе токена |
| **Контекстное окно** | Максимум токенов, которые модель учитывает за один запрос |
| **Pretraining** | Базовое обучение на огромном корпусе текстов |
| **Fine-tuning** | Дообучение под конкретную задачу (меняет веса) |
| **RLHF** | Дообучение по человеческим оценкам ответов |
| **Промпт** | Весь входной текст для модели |
| **Промпт-инжиниринг** | Искусство составления эффективных промптов |
| **Галлюцинация** | Уверенно выданная ложная информация |
| **Knowledge cutoff** | Дата, на которой «заморожены» знания модели |
...
Вы уже знаете, что «голая» LLM умеет только генерировать текст и ограничена своим контекстом. Следующий шаг — дать ей руки и инструменты. Так появляются агенты.
