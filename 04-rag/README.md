...
По сути поиск top-K — это та же `cos_sim` из [раздела 02](../02-llm/README.md#3-embeddings-текст-как-числа), применённая ко всей базе:

```python
# Иллюстративно: найти top-K ближайших по смыслу чанков
def search(query_vec: np.ndarray, db: list[tuple], top_k: int = 4) -> list[tuple]:
    ranked = sorted(db, key=lambda row: cos_sim(query_vec, row[0]), reverse=True)
    return ranked[:top_k]  # db: список (вектор, текст чанка)
```
...
**Практика:** [упражнения к разделу 04 →](../exercises/04-rag/README.md)
