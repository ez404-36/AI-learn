...
|--------|----------------------|
| **RAG** | Поиск релевантных фрагментов + подстановка их в промпт перед генерацией |
| **Retrieval** | Этап поиска релевантных фрагментов |
| **Векторная БД** | База, хранящая векторы и ищущая ближайшие по смыслу |
| **Semantic search** | Поиск по смыслу, а не по точному совпадению слов |
| **Индексация** | Подготовка базы знаний: загрузка → чанкинг → эмбеддинг → сохранение |
| **Чанк (chunk)** | Фрагмент документа — единица индексации и поиска |
| **Overlap** | Перекрытие соседних чанков, чтобы не терять контекст на границах |
| **top-K** | Сколько ближайших фрагментов вернуть при поиске |
| **Re-ranking** | Точная пересортировка найденных кандидатов |
| **Hybrid search** | Сочетание семантического и ключевого поиска |
| **Agentic RAG** | RAG, где агент сам управляет поиском |
