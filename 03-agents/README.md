...
Вы уже знаете, что MCP — протокол подключения инструментов к LLM. Теперь подробности.

**MCP (Model Context Protocol)** — открытый стандарт (от Anthropic), который описывает **единый способ** подключать к LLM внешние инструменты, данные и сервисы. Это решает проблему «зоопарка интеграций».

### Какую проблему решает

Без стандарта каждый инструмент подключается к каждой модели по-своему: для 5 моделей и 10 инструментов нужно 50 разных интеграций. MCP вводит общий «разъём» — как USB: один стандартный порт, к которому подходит любое совместимое устройство.
...
Вы знаете, что ACP — протокол общения между агентами. Разберём, зачем он нужен и чем отличается от MCP.
...
|--------|----------------------|
| **ИИ-агент** | Система на базе LLM, которая планирует и действует в цикле через инструменты |
| **Инструмент (tool)** | Функция, которую агент может вызвать (поиск, API, файлы и т.д.) |
| **Tool calling** | Механизм, которым LLM запрашивает вызов инструмента (выполняет рантайм, не модель) |
| **ReAct** | Паттерн «рассуждение → действие → наблюдение» в цикле |
| **MCP** | Стандарт подключения инструментов и данных к LLM («USB для AI») |
| **MCP-сервер** | Программа, предоставляющая инструменты/данные по протоколу MCP |
| **ACP** | Стандарт общения между агентами |
| **Мультиагентная система** | Несколько агентов, совместно решающих задачу |
| **Оркестратор** | Главный агент, раздающий подзадачи исполнителям |
